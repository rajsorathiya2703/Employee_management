...
const columns = [
  ...
  { 
    ...
    cell: ({ row }) => {
      ...
      if (status === 'Approved') {
        return <span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full text-xs font-semibold">Approved</span>;
      }
      if (status === 'Pending') {
        return <span className="bg-amber-50 text-amber-600 px-3 py-1 rounded-full text-xs font-semibold">Pending</span>;
      }
      return status;
    }
  },
...
const data = [
  { id: 1, date: '24 May 2026', type: 'Punch-Out Adjustment', reason: 'Fingerprint scanner mismatch on punch-out', status: 'Approved' },
  { id: 2, date: '18 May 2026', type: 'Full Day Present', reason: 'Official client visit offsite', status: 'Pending' }
];
