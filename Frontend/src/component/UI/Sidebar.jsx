      ...
      <div className="relative z-10 flex flex-col h-full">
          ...
          <div className={clsx(
            ...
            <img 
              ...
              className={clsx(
                ...
                isOpen ? "h-auto w-full object-contain" : "h-10 w-10 object-cover object-left rounded-md"
              )}
            />
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 pb-6 flex flex-col gap-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {navItems.map((item) => {
            ...
            const isExactActive = location.pathname === item.path || (item.path === '/' && location.pathname === '/dashboard');
            ...
            return (
              <div key={item.name} className="flex flex-col">
                <Link
                  ...
                  onClick={(e) => item.hasSubmenu ? toggleSubmenu(item.name, e) : null}
                ...
                >
                  <div className={clsx("flex items-center", isOpen ? "gap-3" : "justify-center")}>
                    <Icon size={isOpen ? 20 : 22} className={clsx(
                      "shrink-0", 
                  ...
                  {item.hasSubmenu && isOpen && (
                    isMenuOpen 
                      ...
                      : <ChevronRight size={18} className={isActive ? "text-white" : "text-slate-500 group-hover:text-slate-300"} />
                  )}
                </Link>
                  ...
                  <div className="flex flex-col gap-1 mt-1 mb-2 ml-4 pl-4 border-l border-white/10">
                    {item.subItems?.map(sub => {
                      ...
                      return (
                        ...
                        >
                          <Circle size={8} className={isSubActive ? "fill-indigo-400 text-indigo-400" : "text-slate-600"} />
